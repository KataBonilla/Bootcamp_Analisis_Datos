...
print(y.shape)
print(y)
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42) #train_test_split se encarga de clasificar la parte que es para entrenamiento y la parte que para prueba
print("Los datos del entrenamiento para le modelo")
print(X_train.shape)
...
print(f"Brier Score (Sin calibración): {brier_uncalibrated:.4f}")
...
plt.xlabel("Probabilidad predicha", fontsize=12)
plt.ylabel("Fracción de positivos reales", fontsize=12)
plt.title("Comparacion de Métodos de Calibración", fontsize=15)
plt.legend()
plt.grid(True)
plt.show()
